...
function EMICalculator({ principle, monthlyRate, totalMonths }) {
  return (
    (principle * monthlyRate * (1 + monthlyRate) ** totalMonths) /
...
function generatePaymentTable({ principle, emi, monthlyRate, totalMonths }) {
...
export default function LoadCalculator() {
  ...
  const schedule =
    !Number.isNaN(emi) && emi !== Infinity && principle > 0
        ...
        >
          <div className="mb-6 lg:mb-8 text-center sm:text-left">
            ...
            <p className="mt-2 text-sm sm:text-base text-slate-500">
              EMI (Equated monthly installment) Calculator for Home Loan, Car
              Loan & Personal Loan
            </p>
          </div>
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                ...
                <MetricCard
                  ...
                  value={
                    !Number.isNaN(emi) && emi !== Infinity && principle > 0
                      ? DollarFormatter(totalMonths * emi)
                      : "--"
                  }
                ...
                <tbody className="divide-y divide-[#2d2d2d] bg-[#1a1a1a] font-mono text-xs text-slate-300 text-center">
                  {schedule.length > 0 ? (
                    schedule.map((row, idx) => (
                      ...
                      >
                        <td className="px-6 py-3 font-medium text-slate-200">
                          {toOrdinalNumber(idx + 1)} (
                          {toOrdinalNumber(Math.ceil((idx + 1) / 12))} year)
                        </td>
                    ...
                    <tr>
                      ...
                      >
                        Enter a valid loan amount and period to view the
                        schedule.
                      </td>
                    </tr>
                  )}
