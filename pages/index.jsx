...
const TOOLS = [
  { id: 'json_beautifier', name: 'JSON Beautifier', description: 'Format, validate and beautify your JSON data instantly.', icon: Code, color: 'text-amber-600', bg: 'bg-amber-100/50', border: 'hover:border-amber-500/30 group-hover:shadow-amber-500/10' },
  { id: 'base_64_encode_decoder', name: 'Base64 Encoder', description: 'Encode and decode Base64 strings with a developer-friendly interface.', icon: Hash, color: 'text-indigo-600', bg: 'bg-indigo-100/50', border: 'hover:border-indigo-500/30 group-hover:shadow-indigo-500/10' },
  { id: 'str_len', name: 'String Length', description: 'Real-time string character counting utility.', icon: Type, color: 'text-blue-600', bg: 'bg-blue-100/50', border: 'hover:border-blue-500/30 group-hover:shadow-blue-500/10' },
  { id: 'emi', name: 'EMI Calculator', description: 'Calculate Equated Monthly Installments for various loans.', icon: Calculator, color: 'text-teal-600', bg: 'bg-teal-100/50', border: 'hover:border-teal-500/30 group-hover:shadow-teal-500/10' },
  { id: 'kh_tax', name: 'Salary Tax Calc', description: 'Reverse calculate gross salary from expected net income in Cambodia.', icon: CalendarDays, color: 'text-emerald-600', bg: 'bg-emerald-100/50', border: 'hover:border-emerald-500/30 group-hover:shadow-emerald-500/10' },
];
      ...
      <div className="pb-12 xl:pb-24">
        ...
        <div className="relative py-16 sm:py-24 xl:py-32 overflow-hidden rounded-3xl mb-12">
          ...
          <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/60 backdrop-blur-md border border-slate-300/50 text-slate-700 text-xs font-semibold tracking-wide uppercase mb-8 shadow-sm">
               <Sparkles className="w-4 h-4 text-indigo-500" />
               All-in-one Toolkit
            </motion.div>
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-5xl sm:text-7xl font-bold tracking-tighter text-slate-900 mb-6 leading-tight">
            ...
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="max-w-2xl mx-auto text-lg sm:text-xl text-slate-600 mb-10 leading-relaxed">
        ...
        <div id="tools-grid" className="scroll-mt-8">
           <div className="flex items-center justify-between mb-8">
              <h2 className="text-2xl font-bold tracking-tight text-slate-900">Available Utilities</h2>
           </div>
              ...
              {TOOLS.map((tool, idx) => {
                 ...
                 return (
                   <Link href={item.page} key={tool.id}>
                     ...
                     >
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${tool.bg} ${tool.color} mb-6 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300`}>
        ...
        <div className="mt-24 sm:mt-32 pt-16 relative">
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-12 sm:gap-8">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex flex-col items-center text-center">
                 ...
                 <h4 className="text-lg font-semibold text-slate-900 mb-3 tracking-tight">Blazing Fast</h4>
                 <p className="text-sm text-slate-500 leading-relaxed max-w-xs">No server roundtrips. Everything processes instantly right inside your browser session.</p>
              </motion.div>
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="flex flex-col items-center text-center">
                 ...
                 <h4 className="text-lg font-semibold text-slate-900 mb-3 tracking-tight">Privacy First</h4>
                 <p className="text-sm text-slate-500 leading-relaxed max-w-xs">Your data never leaves the device. Safely paste confidential JSON or access local utilities securely.</p>
              </motion.div>
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="flex flex-col items-center text-center">
                 ...
                 <h4 className="text-lg font-semibold text-slate-900 mb-3 tracking-tight">Modern Tech Stack</h4>
                 <p className="text-sm text-slate-500 leading-relaxed max-w-xs">Built upon Next.js, Tailwind CSS, and Framer Motion for a fluid, application-like experience.</p>
              </motion.div>
           </div>
        </div>

        <div className="mt-24 pt-8 text-center border-t border-slate-200/50">
          <p className="text-xs text-slate-400 font-medium">Crafted with attention to detail.</p>
        </div>
      </div>
    </Layout>
  );
