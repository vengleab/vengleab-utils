  ...
  try {
    ...
    if (thead) {
      ...
      if (theadGrid.length > 0) {
        ...
        headerRow = [];
        for (let colIdx = 0; colIdx < maxHeaderCols; colIdx += 1) {
          const columnValues = [];
          for (let rowIdx = 0; rowIdx < theadGrid.length; rowIdx += 1) {
...
export function generateHTMLTable(data, hasHeader, styleType = 'tailwind') {
  ...
  const renderCell = (cell) => {
    if (styleType === 'inline') {
      return `      <td style="${tdClass}">${escapeHTML(cell)}</td>\n`;
    }
    if (tdClass) {
      return `      <td class="${tdClass}">${escapeHTML(cell)}</td>\n`;
    }
