    ...
    >
      <div className="flex items-center gap-2">
        {Icon && (
          <Icon
            size={14}
            className={value ? "text-amber-400" : "text-slate-500"}
          />
        )}
        <span
    ...
    <div className="flex flex-col h-full min-h-[400px]">
      ...
      >
        <div className="flex items-center gap-4">
            ...
            >
              {THEMES.map(t => (
                <option
                  key={t}
                  value={t}
                  className={settings.isLightMode ? "bg-white" : "bg-slate-900"}
                >
                  {t}
                </option>
              ))}
          ...
          <div
            className={`h-4 w-px ${
              settings.isLightMode ? "bg-slate-200" : "bg-slate-800"
            }`}
          />
          <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Settings
              size={14}
              className={settings.showSettings ? "rotate-180" : ""}
            />
            Settings
            {settings.showSettings ? (
              <ChevronDown size={14} />
      ...
      {settings.showSettings && (
        <div
          className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6 p-4 rounded-xl border transition-all duration-300 ${
            settings.isLightMode
        ...
        >
          ...
          <div className="sm:col-span-2 lg:col-span-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 mt-2">
            <ToggleField
              label="Data Types"
              icon={Type}
              value={settings.displayDataTypes}
              onChange={val => updateSetting("displayDataTypes", val)}
            />
            <ToggleField
              label="Object Size"
              icon={Box}
              value={settings.displayObjectSize}
              onChange={val => updateSetting("displayObjectSize", val)}
            />
            <ToggleField
              label="Clipboard"
              icon={Clipboard}
              value={settings.enableClipboard}
              onChange={val => updateSetting("enableClipboard", val)}
            />
            <ToggleField
              label="Editable"
              icon={MousePointer2}
              value={settings.onEdit}
              onChange={val => updateSetting("onEdit", val)}
            />
          </div>
        </div>
      )}
      ...
      {/* JSON Display Area */}
      <div
        className={`flex-1 overflow-auto rounded-lg transition-colors duration-300 ${
          settings.isLightMode
            ? "bg-[#fcfcfc] border border-slate-200 shadow-inner"
            : "bg-slate-900/30"
        }`}
      >
        <ReactJson
      ...
      {(settings.onEdit || settings.onAdd) && (
        <div
          className={`mt-4 p-3 rounded-lg border text-[11px] italic flex items-start gap-2 ${
            settings.isLightMode
              ? "bg-blue-50 border-blue-100 text-blue-600"
              : "bg-blue-500/5 border-blue-500/10 text-blue-400/80"
          }`}
        >
          ...
          <div>
            <p>
              <strong>Editing enabled:</strong> Try{" "}
              <code>ctrl/cmd + click</code> to edit. Use <code>Enter</code> to
              submit and <code>Esc</code> to cancel.
            </p>
          </div>
        </div>
      )}
