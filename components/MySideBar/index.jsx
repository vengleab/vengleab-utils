...
export default function MySideBar({ sidebarOpen, setSidebarOpen, isCollapsed, setIsCollapsed }) {
  const router = useRouter();
  const activePath = router.pathname;

  return (
    <aside className={`fixed inset-y-0 left-0 z-50 bg-[#1e1e1e] text-slate-300 border-r border-[#2d2d2d] flex flex-col transform transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] lg:static lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} ${isCollapsed ? 'w-20' : 'w-72'}`}>
      <div className={`flex items-center gap-3 px-4 h-16 bg-[#1a1a1a] border-b border-[#2d2d2d] shrink-0 transition-all ${isCollapsed ? 'justify-center' : 'px-6'}`}>
        <div className="bg-slate-700/50 p-2 rounded-lg text-slate-300 shrink-0">
          <Calculator className="w-5 h-5" />
        </div>
        {!isCollapsed && <span className="font-bold text-white tracking-wide truncate">DevTools</span>}
        
        <button 
          onClick={() => isCollapsed ? setIsCollapsed(false) : (setSidebarOpen ? setSidebarOpen(false) : null)} 
